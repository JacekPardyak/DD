...
draft <- function(file,
  ...
  if (!is.null(package)) {
    template_path = pkg_file("quarto", "templates", template,
                             package = package)
  ...
  if (!identical(tolower(xfun::file_ext(file)), "Qmd"))
    file <- paste(file, ".qmd", sep = "")
  ...
  file.rename(file.path(dirname(file), "skeleton.qmd"), file)
...
available_templates <- function(package = "rmarkdown", full_path = FALSE) {
  template_folder <- pkg_file("quarto", "templates", package = package)
